...
void *clientThread(void *arg){
  ...
  while((err_rcv = recv(sock_fd, &msg , sizeof(msg), 0)) >0 ){
    ...
    if (msg.c ==-1){//first message, board initialization
      size_x = msg.x;
      size_y = msg.y;
      continue;
    }
    /*
    if(msg.player_id>=currSize)//new client
    {
      currSize*=2;
...
void * playerThread(void * argv){
  ...
  player_thread_init_message init_message=*((player_thread_init_message))
  ...
  while((err_rcv = recv(client_sock_fd, &msg , sizeof(msg), 0)) > 0 ){
    ...
    msg.player_id=player_id;//assign playerID to the message

    for(int i=0;i<currSize;i++){
      if(players[i].client_fd!=client_sock_fd)//é preciso MUDAR para a MAIN thread!!!
...
void *serverThread(void * argc){
  ...
  for(currPlayerCount=0;;currPlayerCount++){
   if(currPlayerCount==currSize){
     currSize*=2;
     players=(player_info*)realloc(players,currSize*sizeof(int));
   }

   //Server waits for a new client to connect
   players[currPlayerCount].client_fd = accept(server_socket,(struct sockaddr *)&client_addr,&size_addr);
   if(players[currPlayerCount].client_fd == -1){
     perror("accept");
     exit(EXIT_FAILURE);
   }

   //Create a new workThread
   pthread_create(&thread_id,NULL,playerThread,&players[currPlayerCount].client_fd);
